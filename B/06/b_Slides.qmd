...
## Adding a Level 2 Predictor {.fs80}
...
> ...the **Intercept** $(\color{#377eb8}{\gamma_{00}})$ (the expected average when $z_j=0$) plus the **Effect of Predictor $z$** $(\color{#377eb8}{\gamma_{01}} z_j)$ plus the **Cluster Deviation** $(\color{#e41a1c}{u_{0j}})$.<br>
...
$$
\begin{align}
\text{MATH}_{ij} &= \beta_{0j} + \color{#4daf4a}{e_{ij}} \\
\beta_{0j} &= \color{#377eb8}{\gamma_{00}} + \color{#377eb8}{\gamma_{01}} \text{PUBLIC}_j + \color{#e41a1c}{u_{0j}}
\end{align}
$$

- $\gamma_{00}$ is the [fixed intercept]{.b .blue}: what is the expected average math score for a private school?
- $\gamma_{01}$ is the [fixed slope]{.b .blue} of PUBLIC: do public schools have higher average math scores than private schools?
## Path Diagram

![](../../diagrams/fixed_slopes_L2.png)
...
## Interpreting the Random Effects {.fs80}
...
## Deriving More Information  {.fs80}
...
## Which ICC Should You Report? {.fs80}
...
## Adding a Level 1 Predictor {.fs80}
...
> ...the **Fixed Population Slope** $(\color{#377eb8}{\gamma_{10}})$.<br>
...
$$
\begin{align}
\text{MATH}_{ij} &= \beta_{0j} + \beta_{1j} \text{FEMALE}_{ij} + \color{#4daf4a}{e_{ij}} \\
\beta_{0j} &= \color{#377eb8}{\gamma_{00}} + \color{#e41a1c}{u_{0j}} \\
\beta_{1j} &= \color{#377eb8}{\gamma_{10}}
\end{align}
$$
...
- $\gamma_{10}$ is the [fixed slope]{.b .blue} of FEMALE: do female students have different math scores than male students, assuming this relationship is identical at every school?
## Path Diagram
...
## Visualizing a Fixed Slope {.fs80}
...
$$
y_{ij} = \underbrace{\color{#377eb8}{\gamma_{00}} + \color{#377eb8}{\gamma_{01}} \mathit{PUBLIC}_j + \color{#377eb8}{\gamma_{10}} \mathit{FEMALE}_{ij}}_{\text{Fixed}} + \underbrace{\color{#e41a1c}{u_{0j}} + \color{#4daf4a}{e_{ij}}}_{\text{Random}}
$$
...
![](../../diagrams/fixed_slopes_L1L2.png)

## Fitting the Combined Model {.fs80}
...
## Interpreting the Combined Model {.fs70}
