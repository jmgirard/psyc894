...
## Parameter Interpretations {.fs80}
...
$$
\begin{align}
\text{STRESS}_{ij} &= \beta_{0j} + e_{ij} \\
\beta_{0j} &= \gamma_{00} + u_{0j}
\end{align}
$$
...
![](../../diagrams/random_intercepts.png)

## Mixed (or Reduced) Equation {.fs80}

$$
\begin{alignat}{2}
&\text{L1:} \quad & y_{ij} &= \beta_{0j} + \color{#e41a1c}{e_{ij}} \\
&\text{L2:} \quad & \beta_{0j} &= \color{#377eb8}{\gamma_{00}} + \color{#4daf4a}{u_{0j}}
\end{alignat}
$$
...
$$
y_{ij} = (\color{#377eb8}{\gamma_{00}} + \color{#4daf4a}{u_{0j}}) + \color{#e41a1c}{e_{ij}}
$$
...
$$
y_{ij} = \underbrace{\color{#377eb8}{\gamma_{00}}}_{\text{Fixed}} + \underbrace{\color{#4daf4a}{u_{0j}} + \color{#e41a1c}{e_{ij}}}_{\text{Random}}
$$
:::
...
## Why are the estimates different? {.fs80}
...
## Deriving Cluster Deviations {.fs80}
...
## Deriving Cluster Intercepts {.fs80}
