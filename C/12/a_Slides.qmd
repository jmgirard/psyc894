...
$$y_{ti} = \beta_{0i} + \beta_{1i} \text{TIME}_{ti} + \beta_{2i} \text{TIME}_{ti}^2 + \color{#4daf4a}{e_{ti}}$$
...
- Using raw polynomials can introduce multicollinearity problems when $\text{TIME}$ and $\text{TIME}^2$ are highly correlated.
...
$$y_{ti} = \beta_{0i} + \beta_{1i} \log(\text{TIME}_{ti}+1) + \color{#4daf4a}{e_{ti}}$$
...
::: {.aside}
***Note:*** In R, `log()` defaults to the natural logarithm (base $e$). We add $+1$ to our time variable for two reasons. First, $\log(0)$ is undefined. Second, $\log(1)=0$, which ensures that our intercept still represents the expected outcome at $\text{TIME}=0$.
:::
...
:::{.fsmaller .pv4}
In our simulated data, the Logarithmic time model has the lowest AIC and BIC, suggesting it provides the best fit for this specific process. Note as well that the weights for both metrics are overwhelmingly in favor of this model.
:::
